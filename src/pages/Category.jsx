...
export default function Category() {
  ...
  const filtered = useMemo(() => {
    return products.filter((p) => parseFloat(p.product_price) <= maxPrice);
  }, [products, maxPrice]);
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-end mb-8">
            <div>
              <h1 className="font-black text-3xl md:text-4xl text-slate-900 dark:text-white tracking-tight">Categories</h1>
              <p className="text-slate-500 dark:text-slate-400 mt-2">Find exactly what you're looking for.</p>
            </div>
          </div>
          ...
          {loading && filtered.length === 0 ? (
            ...
            <div className="text-center py-16">
              <p className="text-slate-500 dark:text-slate-400 col-span-full text-center py-12">No products match your filters.</p>
            </div>
          )}
        </div>
      </div>
